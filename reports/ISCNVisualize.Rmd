...
location.df <- location.df %>%
  ...
  mutate(year=case_when(is.na(as.numeric(yearStr)) ~ as.numeric(yearStr),
                        as.numeric(yearStr) < 20 ~ as.numeric(yearStr) + 2000,
                        as.numeric(yearStr) < 100 ~ as.numeric(yearStr) + 1900,
                        TRUE ~ as.numeric(yearStr)),
         month=as.numeric(monthStr), 
         day=as.numeric(dayStr)) %>%
 select(-contains('Str'))
...
#year bins picked to cover start/end dates and incrament on decade. 
#...No distribution reasoning behind this.
