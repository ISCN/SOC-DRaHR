...
ISCN3 <- SoilDataR::processData_ISCN3(layersDir='../repoData/ISCN_3/Layers/',
                                      metaDir='../repoData/ISCN_3/Meta/',
...
##Add the hard coded units TODO debug this 
hardUnits.df <- ISCN3$ISCNKey %>% filter(type == 'value', !is.na(hardUnit)) %>% select(var, hardUnit)
ISCN3$measure <-  ISCN3$measure %>% 
  left_join(hardUnits.df) %>% 
  mutate(unit = if_else(grepl('^\\s*$', unit), as.character(NA), unit)) %>% #remove blanks
  mutate(unit = if_else(is.na(unit), hardUnit, unit)) #fill in hardcoded units (ie not defined by variables)
```
