...
`r nrow(unique(subset(
    subset(ISCN3$fieldNotes, measurement == 'country' & fieldID %in% noLatLon),
            value=='Unknown')))` sites with no known country nor lat-lon.
...
```{r metals_ext}
ggplot(merge(subset(ISCN3$sample, grepl('(ca|k|mg|na)_ext', measurementID) & value > 0),
             ISCN3$measurement)) +
  geom_histogram(aes(x=value, y=..density..)) + 
  scale_x_log10() + 
  facet_wrap(type~unit, scales='free')

summary(subset(ISCN3$sample, grepl('(ca|k|mg|na)_ext', measurementID) & (value <= 0)))
```
