...
## Mosaik.csv 
```{r Mosaik}
dataFile <- '~/CPEAT_testing/Mosaik.csv' #file path of data 
...
sampleData_2 <- allData[ ,17:23] %>% #identify columns for age
  rename(header='date_type', value='uncal_date_BP', depth='depth_cm') %>% #harmonize header names to match previous samples
  filter(!is.na(value)) #remove missing values
  sampleData <- sampleData_1 %>%
  bind_rows(sampleData_2) #combine with previous samples
```
...
## Stordalen.csv 
```{r Stordalen}
dataFile <- '~/CPEAT_testing/Stordalen.csv' #file path of data 
allData <- read_csv(file=dataFile, skip=1) #big data table of everything
