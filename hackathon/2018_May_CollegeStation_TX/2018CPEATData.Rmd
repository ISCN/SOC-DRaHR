...
metaData <- allData[,1:2] %>% # first two columns as meta data
  filter(!is.na(site_name) & !grepl('^\\s*$', site_name)) %>% # remove empty rows
  bind_rows(data.frame(site_name='site_name', Mosaik='Mosaik', 
                       stringsAsFactors=FALSE)) %>% # transfers                                                                  # miss-identified headers
  spread(key=site_name, value=Mosaik) # convert long to wide

sampleData <- allData[, 5:14] %>% # identify columns for sample reads
  gather(key="header", value = 'value', -depth, -peat_type) %>% # convert to long formate exclude columns that are characters not numerics
...
# create a layer of world
mapWorld <- ggplot2::borders("world", colours='gray80', fill='gray80')
